...
$(document).ready(function() {
  ...
  // Manipula o clique no ícone do carrinho para alternar a classe 'active'
  cartIcon.on('click', function() {
      cart.toggleClass("active");
  });

  // Manipula o clique no botão para fechar o carrinho
  closeCart.on('click', function() {
      cart.removeClass("active");
  });

  // Verifica se o documento está completamente carregado e chama a função 'ready'
    ...
    var title = shopProducts.find('.catalogo-text h4').text();
     ...
     if (cartProductTitles.has(title)) {
        ...
        toastr.warning("Este produto já está no carrinho!", "Atenção", {
           ...
           positionClass: 'toast-top-center', // Posição da notificação (ver opções na documentação)
